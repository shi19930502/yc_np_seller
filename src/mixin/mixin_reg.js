	...
	methods: {
		...
		_ruleLoginName() {
			return {
				validator: (function(rule, value, callback) {
					this._ajax({url: configs.userAPI + 'user/listByEquals', param: {loginName: $.trim(value), sysId: configs.sysID}})
					.then(function(d) {
		...
		_ruleTwoFloat(){	
			return {
				validator: (function(rule, value, callback) {
					var val = $.trim(value)
					if(val.length > 0 && !this.priceFloat.test(val)) {
		              callback(new Error('请输入大于等于零的数字,保留两位小数'))
		            } else {
		              callback();
		            }
				}.bind(this)), trigger: 'blur'
		...
		_ruleTwoFloatPlus(){	
			return {
				validator: (function(rule, value, callback) {
					var val = $.trim(value)
					if(val.length > 0 && (!this.priceFloat.test(val) || val == 0)) {
		              callback(new Error('请输入大于零的数字,最多保留两位小数'))
		            } else {
		              callback();
		            }
				}.bind(this)), trigger: 'blur'
