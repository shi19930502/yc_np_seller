<template>
	<div class="outSockstyle">
		...
		<elemTable :dataList="dataList" :currentPage='pageNum' :pageSize="pageSize" :pageTotal="pageTotal" :loading="dataLoading" @sizeChange="handleSizeChange" @currentChange="handleCurrentChange" @selectionChange="selectionChange">
		    ...
		    <el-table-column prop="stockNumber" label="出库单号">
		    ...
		    <el-table-column label="操作" fixed="right" width="350">
		    	<template slot-scope="scope">
                    <div class="el-button-group">
                        <iconBtn iconClass="el-icon-view" content="查看明细" @click="modalCheck(scope.row)"></iconBtn>
                        <iconBtn iconClass="el-icon-printer" content="打印出库明细" @click="printRow(scope.row)"></iconBtn>
                    </div>
		...
		<stockModal v-if="modalShow" :modal="modalShow" @close="modalClose" :storageListData="storageList" :params="modalParams" @submit="modalSubmit" :stockModalType="modalType" stockType="out"></stockModal>
...
<script>
    ...
    export default {
		...
		mounted() {
			this._searchDic('STOCK_STATUS');
			this._ajax({name: 'datadic/listAll', param: { dataType: 'STOCK_STATUS', extendVal: '2'}})
			.then((function(d) {
		...
		methods: {
			...
			searchTable() {
				...
				return this._ajax({name: 'inoutstock/list', param: this.searchForm, loading: 'dataLoading'}).then(this.renderTable)
			},
            exportExcel() {
            	var dateRange = {
					startTime: '',
				...
				this._ajax({
					...
					.then((function (d) {
                    	...
                    	filedsName = ['出库单号', "类型", "名称、摘要、说明", "品类",'供应商', '合计金额(元)', "备注", "来源单号", "创建时间"],
                    	fileName = '出库管理';
                    ...
                    _this._ajax({url: _this.rootAPI, name: "inoutstockdetail/list", param: param})
                        .then((function (d) {
                            ...
                            d.aaData.forEach((function(em1, index) {
                                ...
                                let priceForDecimal = "-";
                                if(em1.priceForDecimal || el.stockStatus == 1) {
                                	priceForDecimal = em1.priceForDecimal + '元/' + em1.basicUnitName || '';
                                }
                                let changeStockForDecimal = em1.changeStockForDecimal + em1.basicUnitName || '';
                                let priceSkuForDecimal = "-";
                                if(em1.priceSkuForDecimal || el.stockStatus == 1) {
                                	priceSkuForDecimal = em1.priceSkuForDecimal + '元/' + em1.skuUnitName || '';
                                }
                                let stockSkuTemp = em1.changeStockSkuForDecimal;
                                if(!stockSkuTemp) stockSkuTemp = _this._parseFloat(em1.changeStockForDecimal.div(em1.skuWeight), 2)
                                let changeStockSkuForDecimal = stockSkuTemp + em1.skuUnitName || '';
                                let totalAmountForDecimal = em1.totalAmountForDecimal || "0";
								if(el.stockStatus == 0 && !totalAmountForDecimal) {
									totalAmountForDecimal = "-";
								}
                                var item = `
                                            <tr>
                          ...
                          <div style="position: relative;height: 20px;line-height: 20px;font-size: 14px;">
                              <span style="position: absolute;left: 0;">出库类型: ${inoutTypeName}</span>
                               <span style="position: absolute;right: 0;">来源单号: ${sourceId}</span>
                          </div>
                          <div style="position: relative;height: 20px;line-height: 20px;font-size: 14px;">
